document.getElementById('uploadForm').addEventListener('submit', async (e) => {
  ...
  if (result.items.length > 0) {
    openChatAssistant()
    sendMessage("je t'envoie le fichier JSON", result.filename, result.openaiFileId);
  }
...
async function sendMessage(input, filename, openaiFileId) {
    ...
    try {
      ...
      alert('Sending message to the assistant...');
        const response = await fetch('/chat', {
            ...
            body: JSON.stringify({ message: messageText, filename: filename, threadId: threadId, openaiFileId: openaiFileId }),
        });
